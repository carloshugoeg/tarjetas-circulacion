...
export const api = {
  ...
  updatePropietario: (id, payload) => request(`/propietarios/${id}`, { method: "PUT", body: JSON.stringify(payload) }),
  ...
  updateTarjeta: (id, payload) => request(`/tarjetas/${id}`, { method: "PUT", body: JSON.stringify(payload) }),
  deleteTarjeta: (id) => request(`/tarjetas/${id}`, { method: "DELETE" }),
  cambioPropietario: (tarjetaId, propietarioId) =>
    request(`/tarjetas/${tarjetaId}/cambio-propietario?nuevo_propietario_id=${propietarioId}`, { method: "PUT" }),
  cambioMotor: (vehiculoId, motor) =>
    request(`/vehiculos/${vehiculoId}/cambio-motor?nuevo_motor=${encodeURIComponent(motor)}`, { method: "PUT" }),
  cambioColor: (vehiculoId, colorId) =>
    request(`/vehiculos/${vehiculoId}/cambio-color?nuevo_color_id=${colorId}`, { method: "PUT" }),
  desactivarTarjeta: (tarjetaId, estado, motivo) =>
    request(`/tarjetas/${tarjetaId}/desactivar?nuevo_estado=${estado}&motivo=${encodeURIComponent(motivo)}`, {
      method: "PUT",
    }),
  reactivarTarjeta: (tarjetaId, motivo) =>
    request(`/tarjetas/${tarjetaId}/reactivar?motivo=${encodeURIComponent(motivo)}`, {
      method: "PUT",
    }),
};
